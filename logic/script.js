...
// ✅ CARGAMOS LA TARIFA QUE PHP HA PUESTO EN EL HTML
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        console.error(error);
        resultsContainer.innerHTML = 'Error cargando datos.';
    }
});
...
function displayResults(products) {
    if (!products.length) { resultsContainer.innerHTML = 'Sin resultados.'; return; }
    let html = '';
    products.forEach((p, idx) => {
        let precioStd = parseFloat(p.PRECIO_ESTANDAR || p.PRECIO_GRUPO3 || p.PRECIO_CECOFERSA || 0);
        ...
        if (sInfo && sInfo.Estado === 'si') sHtml = '<div class="stock-badge stock-ok">En stock</div>';

        const imgUrl = photosMap.get(String(p.Referencia).toUpperCase());
        const imgHtml = imgUrl ? `<img src="${imgUrl}" class="product-img">` : '';
        ...
        html += `
            <div class="product-card-single">
                ...
                <div class="add-controls">
                    ...
                    <button class="add-budget-btn" onclick="addToBudget('${p.Referencia}', '${p.Descripcion}', ${precioStd}, document.getElementById('qty_${idx}').value, '${netoRaw}', ${extractMinQty(netoRaw)}, ${netVal}, 'En stock')">+ Añadir</button>
                </div>
            </div>`;
    });
